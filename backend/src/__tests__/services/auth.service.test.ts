...
describe('auth.service', () => {
  ...
  describe('register', () => {
    it('creates user with hashed password and returns tokens', async () => {
      const result = await register(`svc-register-${uid}@example.com`, 'password123', 'Test User');
    ...
    it('returned user object does not contain passwordHash', async () => {
      const result = await register(`svc-nohash-${uid}@example.com`, 'password123', 'Test User');

      expect(result.user).not.toHaveProperty('passwordHash');
    });

    it('rejects duplicate email (Prisma P2002)', async () => {
      await register(`svc-dup-${uid}@example.com`, 'password123', 'First');

      await expect(register(`svc-dup-${uid}@example.com`, 'password123', 'Second')).rejects.toThrow();
    });

    it('access token contains userId and email', async () => {
      const result = await register(`svc-token-${uid}@example.com`, 'password123', 'Test User');
    ...
    it('refresh token contains userId and type:refresh', async () => {
      const result = await register(`svc-refresh-${uid}@example.com`, 'password123', 'Test User');
  ...
  describe('login', () => {
    it('succeeds with correct credentials', async () => {
      await register(`svc-login-${uid}@example.com`, 'password123', 'Login User');
    ...
    it('fails with wrong password', async () => {
      await register(`svc-wrongpw-${uid}@example.com`, 'correctpassword', 'User');
