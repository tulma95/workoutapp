  ...
  describe('POST /api/auth/register', () => {
    ...
    it('returns 409 EMAIL_EXISTS for duplicate email (not USERNAME_EXISTS)', async () => {
      ...
      await request(app).post('/api/auth/register').send({
        email: sharedEmail,
        password: 'password123',
        username: username('dup-email-1'),
      });
      const res = await request(app).post('/api/auth/register').send({
        email: sharedEmail,
        password: 'password123',
        username: username('dup-email-2'),
      });
      expect(res.status).toBe(409);
  ...
  describe('PATCH /api/users/me', () => {
    ...
    async function registerUser(tag: string, uname?: string) {
      const res = await request(app).post('/api/auth/register').send({
        email: email(tag),
        password: 'password123',
        username: uname ?? username(tag),
      });
      return res.body.accessToken as string;
    }

    it('can update username via PATCH', async () => {
      token = await registerUser('patch-set');
      const uname = username('patch-set-new');
    ...
    it('returns 409 USERNAME_EXISTS on duplicate username via PATCH', async () => {
      ...
      // Register user B with different username, then try to claim same username
