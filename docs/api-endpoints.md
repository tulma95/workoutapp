...
- `PATCH /api/users/me/password` — body `{ currentPassword, newPassword }` (newPassword min 8). Returns 200 `{ accessToken, refreshToken }` on success — fresh tokens are issued because the tokenVersion is bumped (invalidating all previously issued tokens); 400 `INVALID_PASSWORD` if the current password is wrong. **Rate-limited**: same 30 req / 15 min window as `/api/auth/*` (production only) to prevent brute-forcing the current-password bcrypt check with a leaked token.
- `GET /api/users/me/export` — returns all the user's data (profile, training maxes, workouts+sets, plans+schedule, achievements, friendships, feed events/comments/reactions) as JSON with a `Content-Disposition: attachment` header for download
...
- `GET /api/workouts/:id/previous` - per-exercise last-time performance: `{ [exerciseName]: { weight, reps, completedAt } }` from the AMRAP (top) set of the most recent OTHER completed workout containing that exercise (powers the active-workout "Last time" line). Matches by exercise across **all** the user's completed workouts (plan-agnostic), so after a plan switch it can surface a set from a prior plan.
- `PATCH /api/workouts/:id/sets/:setId` - `{ actualReps?, rpe?, completed? }` (rpe is an optional Int 6-10, nullable to clear; 400 if outside 6-10). Returns the updated set DTO (includes `rpe`).
...
- `DELETE /api/social/feed/:eventId/comments/:commentId` - delete a comment; permitted for the comment author OR the event owner; returns `200 { id: number }`; 403 if neither, 404 if comment or event not found
