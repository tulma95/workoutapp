...
- `PATCH /api/users/me/password` — body `{ currentPassword, newPassword }` (newPassword min 8). Returns 200 `{ accessToken, refreshToken }` on success — fresh tokens are issued because the tokenVersion is bumped (invalidating all previously issued tokens); 400 `INVALID_PASSWORD` if the current password is wrong
