...
- **workouts**: `id, user_id (FK), day_number, plan_day_id (FK plan_days), status ('in_progress'/'completed'/'discarded'), completed_at, created_at` — Index: (user_id, status) optimizes `getCurrentWorkout` (`WHERE user_id AND status = 'in_progress'`), `getHistory` (`WHERE user_id AND status = 'completed'`), and calendar queries (`WHERE user_id AND status != 'discarded'`).
